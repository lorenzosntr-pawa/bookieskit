...
- **Live odds audit harness (#43)** — a reusable
  ...
  markdown odds matrix + JSON sidecar under `docs/audits/`. Two modes:
  `--prematch <seeds…>` (given upcoming fixtures, prematch path) and `--live`
  (auto-discovers in-play events, live feed). Each market×book is classified
  MAPPED+PRICED / NOT OFFERED, with a per-book MIS-MAP review surface (raw
...
- Extended **corner** coverage + two new canonical soccer **booking (cards)** markets (increment 2 of corner & booking mapping, #22). Ids/labels taken from real captured fixtures, never guessed:
  - `1x2_corners_ft` now also mapped on **Bet9ja** (key `S_TEAMCORNER`, outcomes `1`/`X`/`2`) and **Betway** (market name "Corner 1X2"). The captured Bet9ja event's market metadata (`D.MK`/`D.TRANS`) labels `S_TEAMCORNER` as "Corners - 1X2" with the description "Which team will get the most corners during the game in normal play?" — an unambiguous full-time most-corners 1X2 — resolving the disambiguation that left it `None` in increment 1.
  - `over_under_corners_ft` now also mapped on **Betway** (market name "Total Corners"), from its captured `2way_handicap_ft.json`.
...
- **BetPawa API v3 → v4 (every endpoint was returning 404)** — BetPawa retired the v3 sportsbook API. All five call sites (`get_sports`, `get_countries`, `get_tournaments`, `get_events`, `get_event_detail`) now target `/api/sportsbook/v4/…`, as does `scripts/betpawa_availability_monitor.py`. Verified live: v3 returns `404 NOT_FOUND` for both `categories/list/{sport}` and `events/lists/by-queries` (including an unfiltered all-football query), while v4 returns 200 with the same `withRegions[].regions[].competitions[]` and `responses[].responses[]` shapes the parsers already expect. The mocked test suite could not catch this — it pinned v3 URLs in `respx`, so it stayed green while every live call failed.
- **BetPawa v4 served protobuf instead of JSON** — with `accept: */*`, the v4 `/events/lists/by-queries` endpoint answers `application/x-protobuf;charset=UTF-8`, which httpx cannot decode (`UnicodeDecodeError: invalid start byte`); the categories endpoints return JSON either way, so only event fetches broke. `DEFAULT_HEADERS` now sends `accept: application/json` explicitly, pinned by a regression test.
