...
| `get_event_detail(event_id, live=False)` | GET | `/v1/uo/matches?match_id=...&limit=1` | One event's metadata + SR id. Pass `live=True` to fetch from the live host. |
| `get_event_markets(event_id, live=False)` | (4-call aggregation) | `/v1/uo/matches?match_id=...&sub_type_id=N` × 4 | Full universal market set. Fans out one call per `sub_type_id` ∈ {1, 10, 18, 29} concurrently and stitches their `odds` groups into one match-shaped response. |
...
- **Universal `sub_type_id` mapping:** `1` = 1X2, `10` = Double Chance, `18` = Over/Under, `29` = BTTS.
